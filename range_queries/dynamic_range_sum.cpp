...
class segment
{
    ...
    ll query_tree(int node,int start,int end,int left,int right)
    {
        ...
        int mid=(start+end)/2;
        return query_tree(2*node+1,start,mid,left,right) + query_tree(2*node+2,mid+1,end,left,right);
    ...
    void updatetree(ll node,ll start,ll end,ll k,ll val)
    {
        if(start==end)
        {
            seg[node]=val;
        ...
        int mid=(start+end)/2;
        if(k<=mid)
        updatetree(2*node+1,start,mid,k,val);
        else 
        updatetree(2*node+2,mid+1,end,k,val);
        seg[node]=seg[2*node+1]+seg[2*node+2];
    }

    void update(ll k,ll val)
    {
        updatetree(0,0,n-1,k,val);
    }
